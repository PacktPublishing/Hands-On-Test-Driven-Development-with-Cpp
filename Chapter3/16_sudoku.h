// 15 with difficulty metrics (solution_count no longer stores a solution)
#ifndef SUDOKU_H_
#define SUDOKU_H_
#include <cstdlib>
#include <iosfwd>
...
class Sudoku {
    ...
    // Check if a given puzzle solution is valid.
    enum state_t { VALID, INVALID, PARTIAL };
    state_t verify() const;
...
class SudokuSolver {
    public:
    // Initialize the solver for a particular puzzle.
    explicit SudokuSolver(const Sudoku& S);
    // Return the solved puzzle with all cells filed, or no value if the
    // solution does not exist.  If multiple solutions exist, one arbitrarily
    // chosen solution is returned.
    std::optional<Sudoku> solve();
    // Count solutions for a sudoku puzzle.
    size_t solution_count();
    // Puzzle complexity metric (computed by solving the puzzle).
    struct complexity_t {
        size_t max_depth;
        size_t search_count;
    ...
    private:
    Sudoku puzzle;
    std::optional<Sudoku> solution;
    size_t count;
    complexity_t cmplx;
    bool solve_called;
    bool count_called;
    std::optional<Sudoku> solve(const Sudoku& S, size_t depth);
    void solution_count(const Sudoku& S);
};
#endif // SUDOKU_H_
